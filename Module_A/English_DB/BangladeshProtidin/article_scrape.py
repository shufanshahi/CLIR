...
for i, url in enumerate(links, 1):
    try:
        ...
        # Extract date from specific xpath text node
        date_texts = tree.xpath('/html/body/main/section/div[2]/div[1]/div[1]/div[1]/text()')
        date_str = ''.join(date_texts).strip() if date_texts else ''
        
        # Clean up and parse the date string
        # Remove "Publish:" prefix and clean whitespace
        date_str = re.sub(r'Publish:\s*', '', date_str).strip()
        
        # Parse date string to datetime
        try:
            # Format: 21:46, 05 Dec, 2025
